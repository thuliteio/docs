...
You can find an exhaustive list of all changes in [Thulite' changelog](https://github.com/gethyas/hyas/blob/main/CHANGELOG.md), and important instructions for upgrading to each new [major version](#major-changes) in our [upgrade guides](#upgrade-guides).
...
To install a specific [version of Thulite](https://www.npmjs.com/package/gethyas?activeTab=versions) or integrations, use the appropriate command for your package manager.
...
The main Thulite documentation pages are always __accurate for the latest released version of Hyas__. They do not describe or compare to how things worked in previous versions, nor do they highlight updated or changed behavior.
...
When Thulite issues a "patch" version, the last number increases. (e.g. `gethyas@2.4.1` -> `gethyas@2.4.2`)
