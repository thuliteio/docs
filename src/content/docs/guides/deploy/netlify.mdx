---
title: Deploy your Hyas Site to Netlify
description: How to deploy your Hyas site to the web on Netlify.
type: deploy
stub: false
service: Netlify
---
...
[Netlify](https://netlify.com) offers hosting and serverless backend services for web applications and static websites. Any Hyas site can be hosted on Netlify!
...
4. Confirm your build command (`hyas build` or `npm run build`)
    ...
    The CLI will automatically detect the build settings (`hyas build` or `npm run build`) and deploy directory (`public`), and will offer to automatically generate a [`netlify.toml` file](#netlifytoml-file) with those settings.
...
If you are using a legacy [build image](https://docs.netlify.com/configure-builds/get-started/#build-image-selection) (Xenial) on Netlify, make sure that your Node.js version is set. Hyas requires `v18.14.1` or higher.
...
No special configuration is required to use Netlify Functions with Hyas. Add a `netlify/functions` directory to your project root and follow [the Netlify Functions documentation](https://docs.netlify.com/functions/overview/) to get started!
